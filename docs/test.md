...
- 工作站报告审核区“版本记录”可打开版本历史；无版本时显示空状态，保存审核后显示版本列表。
- `/workspace?image_id=<id>` 能直接打开指定病例；从首页最近病例和患者历史病例进入时应选中同一病例。
- `/patients` 患者列表、搜索、新建/编辑、患者病例时间线、报告预览抽屉、版本历史和工作站深链接正常。
...
11. 打开报告版本历史，确认已生成 AI 草稿快照和 `doctor_reviewed` 版本。
12. 使用具备 `finalize:reports` 的账号正式确认报告。
13. 再次打开报告版本历史，确认新增 `finalized` 版本。
14. 返回患者档案，打开该病例的报告预览，确认报告状态、AI 草稿、医生意见、更新时间和版本历史展示正确。
15. 通过患者历史病例的“打开病例”进入 `/workspace?image_id=<id>`，确认工作站选中同一病例。
16. 进入审计中心，使用“报告审核”“正式确认”“数据集导入”“模型评估”“患者更新”等快捷筛选验证日志刷新。
...
- 只读医生可以查看患者历史报告预览和报告版本历史，但不能保存审核意见或正式确认。
