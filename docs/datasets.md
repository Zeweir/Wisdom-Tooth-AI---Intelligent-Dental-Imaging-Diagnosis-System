...
- 第二阶段支持本地目录登记、手动统计录入、小型 zip 样本包上传和公开 zip 直链下载；真实大型公开数据不要提交到仓库。
- zip 上传和 URL 下载第一版只做 MinIO/本地存储和文件名索引，不做完整 COCO/VOC/DICOM 解析。
- Kaggle 页面地址通常不是 zip 直链；如需自动下载 Kaggle 私有/登录数据集，后续应单独接入 Kaggle API 凭据。
